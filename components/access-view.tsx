...
import { getAccessView, type UserEntry, type Resource } from "@/app/actions";
...
function UserDropdown({ users, selectedId, onSelect }: {
  users: UserEntry[];
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map((u) => (
                ...
                >
                  ...
                  <span className={cn("shrink-0 rounded-full px-2 py-0.5 text-xs font-medium ring-1", roleBadgeColors[u.role] ?? "bg-muted text-muted-foreground ring-border")}>
                    {u.role}
                  </span>
                </button>
              ))
